...
class Indicators():
    ...
    def rsi(self,period:int, method:str="wilders")-> pd.DataFrame:
        ...
        self._frame['rsi']=np.where(relative_strength_index==0,100,100.0-(100/(1.0+relative_strength)))
    ...
    def sma(self, period:int)->pd.DataFrame:
        ...
        self._frame[column_name]=self._price_groups['close'].transform(
            lambda x: x.roling(window=period).mean()
        )
        return self._frame
    ...
    def refresh(self):
        ...
        #llop through all stored indicators
