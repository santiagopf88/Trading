...
class Trade():
    ...
    def new_trade(self,trade_id:str,order_type:str,side:str,enter_or_exit:str, price:float=0.00,stop_limit_price:float=0.00)->dict:
        ...
        if enter_or_exit=='enter':
            self.eneter_or_exit_opposite='exit'        
        elif enter_or_exit=='exit':
            self.eneter_or_exit_opposite='enter'
    ...
    def good_till_cancel(self,cancel_time:datetime)->None:
        self.order['duration']="GOOD TILL CANCEL"
    ...
    def add_box_range(self,profit_size:float=0.00,stop_size:float=0.00,stop_loss:bool=False, percentage:bool=False,):
        ...
        if not stop_loss:
            self.add_stop_loss(stop_size=stop_size,percentage=percentage)
    ...
    def add_stop_loss(self,stop_size:float,percentage:bool=False)->bool:
        ...
        stop_loss_order={
            "orderStrategyType": "SINGLE",
            "orderType": self.order_types[self.order_type],
            "session": "NORMAL",
            "duration": "DAY",
            "stopPrice":new_price,
            "orderLegCollection": [
    ...
    def _convert_to_trigger(self):
        if self.order and self._triggered_added==False:
            self.order["orderStategyType"]="TRIGGER"
    ...
    def _generate_order_id(self)->str:
        if self.order:
            order_id="{sumbol}_{side}_{enter_or_exit}_{timestamp}"
